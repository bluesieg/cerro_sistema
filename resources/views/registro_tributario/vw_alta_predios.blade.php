            ...
            <section id="widget-grid" class="">
                <div class="row">
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="margin-bottom: -12px">
                        <div class="well well-sm well-light">
                            <h1 class="txt-color-green"><b>
                                    <h1 class="txt-color-green"><b>ALTA DE PREDIOS</b></h1>
                            <div class="row">
                                    ...
                                    <div class="text-right">
                                        @if( $permisos[0]->btn_new ==1 )
                                            <button type="button" class="btn btn-labeled bg-color-greenLight txt-color-white" onclick="nuevo_dpredios();">
                                                <span class="btn-label"><i class="glyphicon glyphicon-plus-sign"></i></span>GENERAR ALTA
                                            </button>
                                        @else
                    ...
                    <article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                        ...
                        <table id="tabla_alta_predios"></table>
                        <div id="pager_tabla_alta_predios"></div>
                    </article>
                </div>
            </section>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        var pageWidth = $("#tabla_alta_predios").parent().width() - 100;
        contrib_global=0;
        
        jQuery("#tabla_alta_predios").jqGrid({
            ...
            rowNum: 20,sortname: 'id_trans', viewrecords: true, caption: 'PREDIOS TRANSFERIDOS', align: "center",
            colModel: [
            ...
            pager: '#pager_tabla_alta_predios',
            rowList: [10, 20],
            gridComplete: function () {
                    var idarray = jQuery('#tabla_alta_predios').jqGrid('getDataIDs');
                    if (idarray.length > 0) {
                    var firstid = jQuery('#tabla_alta_predios').jqGrid('getDataIDs')[0];
                            $("#tabla_alta_predios").setSelection(firstid);
                        }
                },
            onSelectRow: function (Id){
                $('#current_id').val($("#tabla_alta_predios").getCell(Id, "id_trans"));
        ...
        $(window).on('resize.jqGrid', function () {
            $("#tabla_alta_predios").jqGrid('setGridWidth', $("#content").width());
        });
        ...
        jQuery("#tabla_predios").jqGrid({
        url: 'obtener_predios_alta?id_contrib='+0+'&anio='+0,
        datatype: 'json', mtype: 'GET',
        height: 200, width: 950,
        toolbarfilter: true,
        colNames: ['ID_PRED', 'CODIGO VIA', 'SECTOR', 'MANZANA', 'LOTE', 'REFERENCIA'],
        rowNum: 12, sortname: 'id_pred_contri', sortorder: 'asc', viewrecords: true, caption: 'PREDIOS DEL CONTRIBUYENTE', align: "center",
        colModel: [
        ...
        pager: '#pager_tabla_predios',
        rowList: [10, 20],
        gridComplete: function () {
            var idarray = jQuery('#tabla_predios').jqGrid('getDataIDs');
                    if (idarray.length > 0) {
                    var firstid = jQuery('#tabla_predios').jqGrid('getDataIDs')[0];
                            $("#tabla_predios").setSelection(firstid);
                        }            
   
        },            
        onSelectRow: function (Id){
                $('#current_id_tabla').val($("#tabla_predios").getCell(Id, "id_pred_contri"));

        }
    });  
    
    jQuery("#table_nuevo_contribuyente").jqGrid({
            url: 'obtener_contribuyentes?dat=0',
            datatype: 'json', mtype: 'GET',
            height: 300, width: 480,
            toolbarfilter: true,
            ...
            pager: '#pager_table_nuevo_contribuyente',
            rowList: [10, 20],
            gridComplete: function () {
                    var idarray = jQuery('#table_nuevo_contribuyente').jqGrid('getDataIDs');
                    if (idarray.length > 0) {
                    var firstid = jQuery('#table_nuevo_contribuyente').jqGrid('getDataIDs')[0];
                            $("#table_nuevo_contribuyente").setSelection(firstid);
                        }
                    jQuery('#table_nuevo_contribuyente').jqGrid('bindKeys', {"onEnter": function (rowid) { traer_informacion_contribuyente(rowid); }});
                },
            onSelectRow: function (Id){
                $('#current_id_tabla_nuevo_contrib').val($("#table_nuevo_contribuyente").getCell(Id, "id_contrib"));

            }
        });
        
        var globalvalidador1=0;
        $("#dlg_nuevo_contribuyente").keypress(function (e) {
                    if (e.which == 13) {
                        if(globalvalidador1==0)
                        {
                            fn_bus_nuevo_contrib();
                            globalvalidador1=1;
                        }
                        else
                        {
...
<script language="JavaScript" type="text/javascript" src="{{ asset('archivos_js/registro_tributario/altapredios.js') }}"></script>
    ...
    <div class="widget-body">
        <div  class="smart-form">
            <div class="panel-group">
                ...
                <div class="panel panel-success">
                    <div class="panel-heading bg-color-success">.:: Datos del Predio ::.</div>
                        ...
                        <fieldset>
                            ...
                            <section>                                
                                <article class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="margin-top:5px; margin-bottom: 10px; padding: 0px !important">
                                    ...
                                    <table id="tabla_predios"></table>
                                    <div id="pager_tabla_predios">                                        
                                    </div>
                                </article>
                            </section>
                            ...
                            <section>
                                ...
                                <label class="textarea">
                                    <textarea id="dlg_glosa" rows="2" placeholder="DESCRIPCION DE LA GLOSA" class="input-sm text-uppercase"></textarea>                                    
                                </label>                                       
                            </section>
                            
                            <section>
                                    <label class="label">Nuevo Contribuyente:</label>
                                    <label class="input">
                                        <input type="hidden" id="dlg_hidden_nuevo_contribuyente">
                                        <input id="dlg_nuevo_contribuyente" type="text" placeholder="ESCRIBIR NOMBRE DEL CONTRIBUYENTE" class="input-sm text-uppercase">
                                    </label>
                            </section>
...
<div id="dlg_bus_nuevo_contribuyente" style="display: none;">
    <article class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="margin-top:5px; margin-bottom: 10px; padding: 0px !important">
        <input type="hidden" id="current_id_tabla_nuevo_contrib" value="0">
        <table id="table_nuevo_contribuyente"></table>
        <div id="pager_table_nuevo_contribuyente"></div>
    </article>
</div> 
@endsection
