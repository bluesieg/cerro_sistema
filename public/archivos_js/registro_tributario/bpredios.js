...
function seleccionar_filtro(){
    opcion = $("#select_filtro option:selected").attr("value");
    seleccion = $("#select_filtro option:selected").html();
    if (opcion == 0) {
        $("#dlg_direccion_span").html('-'+'<i class="fa fa-map-marker" style="width: 135px"></i>');
        $("#dlg_direccion").attr('disabled', 'disabled');
    }else{
        $("#dlg_direccion_span").html(seleccion+'<i class="fa fa-map-marker" style="width: 65px"></i>');
        $("#dlg_direccion").removeAttr('disabled');
    }
    
    if (opcion == 1) {
     autocompletar_via('dlg_direccion');  
     limpiar();
    }
    else if (opcion == 2) {
     autocompletar_hab_urb('dlg_direccion');
     limpiar();
    }
...
function fn_buscar_predios(){
    ...
    direccion = $("#hidden_dlg_direccion").val();
    ...
    $.ajax({
        url: 'buscar_predios?direccion=' + direccion,
        type: 'GET',
        success: function (data) {
            if (data.msg == 'si'){
                  MensajeExito('LA BUSQUEDA FUE REALIZADA CON EXITO');      
            }else{

            }
        },
        error: function (data) {
            mostraralertas('* Error al Hacer la Busqueda.<br>* Contactese con el Administrador.');
        }
    });
}
...
jQuery("#tabla_predio").jqGrid({
        ...
        toolbarfilter: true,
        colNames: ['ID','CODIGO CATASTRAL', 'DIRECCION', 'PROPIETARIO'],
        rowNum: 12, sortname: 'id_pred', sortorder: 'asc', viewrecords: true, caption: 'BUSCAR PREDIOS', align: "center",
        colModel: [
