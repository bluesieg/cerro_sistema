...
class Res_DeterminacionController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        for($i=$cuenta[0]->anio_fis;$i<=date("Y");$i++)
        {
            ...
            if(count($id_tributo)>=1)
            {
                DB::select("update adm_tri.cta_cte set id_rd=".$rd->id_rd." where id_pers=".$cuenta[0]->id_contrib." and ano_cta='".$i."' and id_tribu=".$id_tributo[0]->id_tributo);
                $id_pred_anio=$this->create_predio_fis($rd->id_rd,$i);
    ...
    public function create_predio_fis($id_rd,$anio)
    {
        ...
        if(count($id_car)>=1)
        {
            ...
            {
                $predio_anio=$this->descativar_predio_anio($pre->id_pred,$anio);
    ...
    {
        $arancel = DB::select("select * from catastro.arancel where id_via=".$pre->id_via." and anio='".$anio."' and sec='$pre->sec' and mzna='$pre->mzna'" );
        if(count($arancel)>=1)
        {
